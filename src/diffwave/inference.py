...
def predict(
    ...
    with torch.no_grad():
        ...
        conditioner = None
        if model.params.cond_type == "spec":
            if (
                len(spectrogram.shape) == 2
            ):  # Expand rank 2 tensors by adding a batch dimension.
                spectrogram = spectrogram.unsqueeze(0)
            conditioner = spectrogram.to(device)
            audio = torch.randn(
                spectrogram.shape[0],
        ...
        if model.params.cond_type == "class":
            conditioner = class_embedding.to(device)
...
def main(args):
    if args.spectrogram_path:
        ...
        spectrogram = None
    if args.class_embedding_path:
        class_embedding = torch.from_numpy(np.load(args.class_embedding_path))
    else:
        class_embedding = None
    audio, sr = predict(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-c",
        help="path to a numpy array containing class embeddings",
    )
